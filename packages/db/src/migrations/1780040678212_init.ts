...
const LANGUAGES = [
  ...
  "ce", "ny", "cu", "cv", "kw", "co", "cr", "hr", "cs", "da", "dv", "nl", "dz",
  "en", "eo", "et", "ee", "fo", "fj", "fi", "fr", "fy", "ff", "gd", "gl", "lg",
  "ka", "de", "el", "kl", "gn", "gu", "ht", "ha", "he", "hz", "hi", "ho", "hu",
  "is", "io", "ig", "id", "ia", "ie", "iu", "ik", "ga", "it", "jv", "kn", "kr",
  "ks", "kk", "km", "ki", "rw", "ky", "kv", "kg", "kj", "ku", "lo", "la", "lv",
  "li", "ln", "lt", "lu", "lb", "mk", "mg", "ms", "ml", "mt", "gv", "mi", "mr",
  "mh", "mn", "na", "nv", "nd", "nr", "ng", "ne", "no", "nb", "nn", "ii", "oc",
  "oj", "or", "om", "os", "pi", "ps", "fa", "pl", "pa", "qu", "ro", "rm", "rn",
  "ru", "se", "sm", "sg", "sa", "sc", "sr", "sn", "sd", "si", "sk", "sl", "so",
  "st", "su", "sw", "ss", "sv", "tl", "ty", "tg", "ta", "tt", "te", "th", "bo",
  "ti", "to", "ts", "tn", "tr", "tk", "tw", "ug", "uk", "ur", "uz", "ve", "vi",
  "vo", "wa", "cy", "wo", "xh", "yi", "yo", "za", "zu", "es", "es_la", "pt_br",
  "pt_pt", "ja", "ja_ro", "ko", "ko_ro", "zh", "zh_hk", "zh_ro",
] as const
...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    .createType("Flag")
    .asEnum(["OK", "STAFF_ONLY", "VIP_ONLY", "LOCKED"])
    .execute()
  await db.schema
    .createType("Roles")
  ...
  await db.schema
    ...
    .execute()
  await db.schema.createType("Languages").asEnum([...LANGUAGES]).execute()
  await db.schema.createType("Countries").asEnum([...COUNTRIES]).execute()
  ...
  await db.schema
    ...
    .addColumn("createdAt", sql`timestamp(3)`, (c) =>
    ...
    .addColumn("role", sql`"Roles"`, (c) =>
      c.notNull().defaultTo(sql`'USER'::"Roles"`),
    )
    .addColumn("settings", "jsonb", (c) =>
  ...
  await db.schema
    ...
    .addColumn("reading", sql`jsonb[]`, (c) =>
      c.defaultTo(sql`ARRAY[]::jsonb[]`),
    )
    .addColumn("rereading", sql`jsonb[]`, (c) =>
      c.defaultTo(sql`ARRAY[]::jsonb[]`),
    )
    .addColumn("planToRead", sql`jsonb[]`, (c) =>
      c.defaultTo(sql`ARRAY[]::jsonb[]`),
    )
    .addColumn("completed", sql`jsonb[]`, (c) =>
      c.defaultTo(sql`ARRAY[]::jsonb[]`),
    )
    .addColumn("onHold", sql`jsonb[]`, (c) =>
      c.defaultTo(sql`ARRAY[]::jsonb[]`),
    )
    .addColumn("dropped", sql`jsonb[]`, (c) =>
      c.defaultTo(sql`ARRAY[]::jsonb[]`),
    )
    .addColumn("userId", "uuid", (c) =>
      c
  ...
  await db.schema
    .createIndex("_UserFollow_B_index")
    .on("_UserFollow")
    .column("B")
    .execute()
  ...
  await db.schema
    ...
    .addColumn("countryOfOrigin", sql`"MediaCountryOfOrigin"`, (c) =>
      c.notNull(),
    )
    .addColumn("tags", sql`jsonb[]`, (c) => c.defaultTo(sql`'{}'::jsonb[]`))
    .addColumn("flag", sql`"Flag"`, (c) =>
      c.notNull().defaultTo(sql`'OK'::"Flag"`),
    )
  ...
  await db.schema
    .createIndex("_ChapterToGroup_B_index")
    .on("_ChapterToGroup")
    .column("B")
    .execute()
