...
import { TAG_KEYS } from "~/i18n/tags"
...
const getTitles = (manga: Manga) => {
  ...
  rawTitles.push({
    title: manga.localizedTitle.data[manga.localizedTitle.availableLocales[0]!]!,
  ...
  for (const title of rawTitles) {
    const titlesWithSameLanguage = rawTitles.filter((t) => title.language === t.language)
    const titleIndex = titlesWithSameLanguage.findIndex((t) => t.title === title.title)
